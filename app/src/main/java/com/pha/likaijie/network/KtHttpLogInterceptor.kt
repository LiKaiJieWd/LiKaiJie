...
import com.pha.likaijie.network.GZIPUtils.uncompress
import okhttp3.*
import okio.Buffer
...
class KtHttpLogInterceptor(block: (KtHttpLogInterceptor.() -> Unit)? = null) : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        return kotlin.runCatching { chain.proceed(request) }.onFailure {
            it.printStackTrace()
    ...
    private fun logBasicReq(sb: StringBuilder, request: Request, connection: Connection?) {
        sb.append("请求 method: ${request.method}  tag: ${request.tag()} protocol: ${connection?.protocol() ?: Protocol.HTTP_1_1}")
            .append("请求 request decodeUrlStr: ${decodeUrlStr(request.url.toString())}")
            .append("请求 request encodeUrlStr: ${request.url}")
    }
        ...
        when (logLevel) {
            ...
            LogLevel.BODY -> {
                ...
                kotlin.runCatching {
                    ...
                    sb.append(uncompress(peekBody.bytes())?.let { String(it) })
                }.getOrNull()
            }
        }
        sb.append("<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<-<<")
    ...
    private fun logBasicRsp(sb: StringBuffer, response: Response) {
        sb.append("响应 protocol: ${response.protocol} code: ${response.code} message: ${response.message}")
            .append("响应 request decodeUrlStr: ${decodeUrlStr(response.request.url.toString())}")
            .append("响应 request encodeUrlStr: ${response.request.url}")
            .append(
                "响应 sentRequestTime: ${
                    toDateTimeStr(response.sentRequestAtMillis, MILLIS_PATTERN)
                } receivedResponseTime: ${
                    toDateTimeStr(response.receivedResponseAtMillis, MILLIS_PATTERN)
                }"
            )
    }
